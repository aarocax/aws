...
class XpressIDController extends BaseController
{

  private XpressIdHttpClient $xpressIdHttpClient;
  ...
  private static $LITERALES = array(
    'es' => array(
      ...
      "obverseNotFoundText" => "Encaja la parte delantera",
      "reverseNotFoundText" => "Encaja la parte trasera",
      "infoUserDocumentTitle" => '1. Centra tu documento en el marco\n\n 2. Asegúrate de que tu documento no muestra brillos ni sombras.\n\n 3. La fotografía se capturará de forma automática.',
      "repeatText" => "Repetir",
      "infoUserSliderButtonText" => "Continuar",
      "infoUserAliveHeader" => "Antes de comenzar, lee estas instrucciones",
      ...
      "iframeLoaderText" => "Estamos procesando tu información, espera un momento por favor.",
      "videoErrorDefault" => "Lo sentimos, ha habido un error al conectar con tu cámara",
      "message_alive_light" => "Mueve tu cabeza",
      "message_alive_bold" => "LENTAMENTE",
      "stepChallenge" => "Paso",
      "stepOfChallenge" => "de",
      "iframeQrTitle" => "Escanea este código QR con tu Smartphone",
  ...
  public function __construct(XpressIdHttpClient $xpressIdHttpClient)
  {
    parent::__construct();
    $this->xpressIdHttpClient = $xpressIdHttpClient;
  }

  public function getToken(array $data)
  {
    $this->logInfo(json_encode($data));
    $this->response = $this->xpressIdHttpClient->getToken($this->prepareData($data));
  ...
  private function prepareData(array $data)
  {
    ...
    $mobileRedirectURL = $this->mobileRedirectUrl($lang);

    $disableContinueOnDesktop = $this->disableContinueOnDesktop();
    ...
    $ret = [
      ...
      'data' => '{
        ...
        "mobileQrRedirect": "' . $mobileRedirectURL . '",
        "mobileQrParams": {
          "step": "4",
          "hash": "' . $data['hash'] . '"
        },
   ...
   */
  private function checkXpressIDResponse(string $curlResponse)
  {
  ...
  /*
   * Comprueba si se puede realizar la video identificación en el
   * ordenador. Mostrando bajo el QR un botón "Continuar en el ord." 
   * en caso afirmativo o forzando la redirección a móvil tras escanear 
   * el código QR.
   */
  private function disableContinueOnDesktop(): string
  {
    $disable = "true";
    if (
      AppConfig::getInstance()->getValue("BOIDAS_USERNAME") !== null &&
  ...
  /*
   * Retorna la url a donde se redirecciona tras la lectura 
   * del código QR dependiendo del idioma (es/en)
   */
  private function mobileRedirectUrl(string $lang): string
  {
    return ($lang == "es") ? AppConfig::getInstance()->getValue("MOBILE_REDIRECT_URL") : AppConfig::getInstance()->getValue("MOBILE_REDIRECT_URL_EN");
  }
  ...
  private function getEmail(array $data): string
  {
    if (isset($data["atfinity_email"]) && !empty($data["atfinity_email"])) {
      ...
      $email = "";
      $this->logError("No hay un correo para la firma con esign");
    }
    return $email;
  }
  ...
  private function getUserName(array $data): string
  {
    if (isset($data["userName"]) && !empty($data["userName"])) {
      ...
      $userName = "";
      $this->logError("No hay un user name para la firma con esign");
    }
    return $userName;
  }
